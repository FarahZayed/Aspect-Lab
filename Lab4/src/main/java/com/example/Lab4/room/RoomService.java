...
public class RoomService {
    ...
    public List<Room> getAllRooms() {
        ...
        if (rooms != null && !rooms.isEmpty()) {
            try {
                //transfer from json to string as redis must be saved as string
                String roomsJson = objectMapper.writeValueAsString(rooms);
                //Cache_ttl : how musch time the key is redis will be stored
    ...
    @Transactional
    @DistributedLock(keyPrefix = ROOM_LOCK_PREFIX, keyIdentifierExpression = "#id", leaseTime = 15, timeUnit = TimeUnit.SECONDS)
    public Room updateRoom(Long id, UpdateRoomDTO roomDTO) {
        ...
        log.info("Room update for id {}: Starting 10 seconds delay...", id);
        try {
            log.info("Acquiring lock for room {}", id);
            TimeUnit.SECONDS.sleep(10); // Wait for 3 minutes
